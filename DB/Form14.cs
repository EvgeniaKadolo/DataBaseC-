...
namespace DB
{
    public partial class Form14 : Form
    {
        ...
        public Form14(NpgsqlConnection npgsqlConnection)
        {
            InitializeComponent();
            ...
            cmd.CommandText = "SELECT sum(CASE id_type_dish WHEN 1 THEN 1 ELSE 0 END) AS Закуска, sum(CASE id_type_dish WHEN 2 THEN 1 ELSE 0 END) As Десерт, sum(CASE id_type_dish WHEN 3 THEN 1 ELSE 0 END) AS Напиток, sum(CASE id_type_dish WHEN 4 THEN 1 ELSE 0 END) AS Первое, sum(CASE id_type_dish WHEN 5 THEN 1 ELSE 0 END) AS Второе FROM dish;";
